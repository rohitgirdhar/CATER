...
# Using xinleic's c2board to write logs
...
def train(opts):
    ...
    summary_writer = SummaryWriter(log_dir=os.path.join(
        os.path.dirname(checkpoint_dir), 'tensorboard'))
    ...
    for curr_iter in range(start_model_iter, cfg.SOLVER.MAX_ITER):
        ...
        train_meter.calculate_and_log_all_metrics_train(
            curr_iter, train_timer, summary_writer)
        ...
        if (curr_iter + 1) % cfg.TRAIN.EVAL_PERIOD == 0:
            ...
            test_meter.log_final_metrics(curr_iter, summary_writer)
