...
sys.path.insert(0, osp.join(osp.dirname(osp.realpath(__file__)), '../../lib/utils'))
from label_id import parts_to_full_id
        ...
        for i in range(items):
            ...
            for ct in range(crop_times):
                ...
                for j in range(test_start_frame_num):
                    ...
                    if use_full_label_id:
                        # Storing the label_id like this stores the information
                        # which exact clip is being tested. So in the test
                        # code I know where it came from and can train LSTM etc
                        # on top of it.
                        label_id = parts_to_full_id(
                            i, j, ct, test_start_frame_num, crop_times)
                    else:
                        label_id = i
                    ...
                    label_tensor.int32_data.append(label_id)
                    # for label in labels:
...
def main():
    ...
    parser.add_argument("--list_file", type=str, default=None,
                        ...
                        required=True)
    parser.add_argument("--use_full_label_id", action='store_true',
                        help='Set this flag to store the full information, '
                             'including the temporal/spatial crop ID.')

    args = parser.parse_args()
    create_an_lmdb_database(args.list_file, args.dataset_dir,
                            use_full_label_id=args.use_full_label_id)
