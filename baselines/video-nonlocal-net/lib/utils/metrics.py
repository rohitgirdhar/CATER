...
class MetricsCalculator():
    ...
    def log_final_metrics(
            self, model_iter, summary_writer, total_iters=cfg.SOLVER.MAX_ITER):
        ...
        summary_writer.write_scalars(
            {
                'Error/{}'.format(self.split): self.avg_err,
                'Error-top5/{}'.format(self.split): self.avg_err5,
            },
            model_iter + 1,
        )
    ...
    def calculate_and_log_all_metrics_train(
            self, curr_iter, timer, summary_writer):
        ...
        if (curr_iter + 1) % cfg.LOG_PERIOD == 0:
            ...
            summary_writer.write_scalars(
                {
                    'ETA': eta_seconds,
                    'Loss/{}'.format(self.split): cur_loss,
                    'Error/{}'.format(self.split): cur_err,
                },
                curr_iter + 1,
            )

    def calculate_and_log_all_metrics_test(
            self, curr_iter, timer, total_iters):
...
def compute_topk_correct_hits(top_k, preds, labels):
    ...
    top_k = min(top_k, preds.shape[1])  # for binary classification etc
...
def compute_topk_correct_hits_multilabel(top_k, preds, labels):
    """ Compute accuracy in the multi-label case. Note that simple accuracy
    won't make sense here, so will compute differently.
    The preds are normalized predictions, hence can use 0.5 to threshold. """
    # top_k is not used for this setup
    preds_copy = preds.copy()
    preds_copy[preds_copy >= 0.5] = 1
    preds_copy[preds_copy < 0.5] = 0
    return np.mean(preds_copy.astype('int32') == labels)
...
def compute_multi_gpu_topk_accuracy(top_k, split):
    ...
    for idx in range(cfg.ROOT_GPU_ID, cfg.ROOT_GPU_ID + cfg.NUM_GPUS):
        ...
        # rgirdhar: pred was before the softmax, it didn't matter for simple
        # classification, but for sigmoid, I'll take after the activation
        # Added an alias in the loss.py code so I can use the same handle
        activation = workspace.FetchBlob('gpu_{}/activation'.format(idx))
        ...
        if cfg.METRICS.EVAL_FIRST_N and split in ['test', 'val']:
            if cfg.MODEL.MULTILABEL:
                aggr_top_k_correct_hits += \
                    compute_topk_correct_hits_multilabel(
                        top_k, activation[:, :cfg.METRICS.FIRST_N], labels)
            else:
