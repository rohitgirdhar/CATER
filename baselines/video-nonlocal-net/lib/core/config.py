...
# Set to true for multi-label classification setting
__C.MODEL.MULTILABEL = False
...
__C.TEST.OUTPUT_NAME = b'activation'
...
__C.TEST.FORCE_RECOMPUTE_RESULTS = False
...
def assert_and_infer_cfg():
    ...
    # Added by rgirdhar
    # Automatically set the DATASET_SIZE
    # FILENAME_GT is supposed to be the val file, so use that
    __C.TEST.DATASET_SIZE = count_lines(__C.FILENAME_GT)
    __C.TRAIN.DATASET_SIZE = count_lines(__C.FILENAME_GT.replace(
        'val.txt', 'train.txt'))
    logger.warning('Replacing the dataset sizes from what is read from the '
                   'train/test files TRAIN: {} and TEST: {}'.format(
                       __C.TRAIN.DATASET_SIZE, __C.TEST.DATASET_SIZE))
    if __C.TEST.OUTPUT_NAME != 'activation':
        logger.warning('Reverting TEST.OUTPUT_NAME to activation. '
                       'New code aliases the last layer after non-linearity '
                       'to activation, so this should not be required.')
        __C.TEST.OUTPUT_NAME = 'activation'

    # Check if the number of classes matches the metadata for that dataset
    metadata_fpath = osp.join(osp.dirname(__C.DATADIR.rstrip(os.sep)), 'metadata.pkl')
    if osp.exists(metadata_fpath):
        with open(metadata_fpath, 'r') as fin:
            metadata = pkl.load(fin)
        assert len(metadata['classes']) == __C.MODEL.NUM_CLASSES
