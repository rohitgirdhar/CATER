...
class ModelBuilder(cnn.CNNModelHelper):
    ...
    def __init__(self, **kwargs):
        ...
        self.split_dir_name = kwargs.get('split_dir_name', self.split)
        ...
        if 'split' in kwargs:
            del kwargs['split']
        if 'split_dir_name' in kwargs:
            del kwargs['split_dir_name']
    ...
    def build_model(self, node_id=0):
        ...
        self.data_loader = self.CreateDB(
            "reader_" + self.split,
            db=dirname + '/' + self.split_dir_name,
            db_type="lmdb",
            num_shards=1,
            shard_id=node_id,
        )
    ...
    def create_data_parallel_model(
        ...
        def AddVideoInput(model, reader, batch_size):
            ...
            other_kwargs = {}
            if cfg.MODEL.MULTILABEL and cfg.TEST.DATA_TYPE != 'test':
                # Note that when cfg.TEST.DATA_TYPE is 'test', then the
                # label field contains the index to the video and not the
                # label itself. So I should be using it like single integer
                # label. The labels for evaluation will be read in the
                # test py file.
                other_kwargs['multiple_label'] = 1
                other_kwargs['num_of_labels'] = cfg.MODEL.NUM_CLASSES
